...
The system MUST create `HeaderComponent` (with app title and navigation links), `FooterComponent`, and `HomeComponent`. `AppComponent` MUST use the header/footer shell wrapping a `<router-outlet>`. Routes MUST include a lazy-loaded home route, lazy-loaded auth routes (`/login`, `/register`, `/recuperar`, `/reset-password`), and a wildcard redirect to `/`.
