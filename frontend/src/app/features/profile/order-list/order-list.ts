...
export class OrderListComponent implements OnInit, OnDestroy {
  ...
  getStatusClasses(status: OrderStatus): string {
    const map: Record<string, string> = {
      pending: 'ad-status is-pending',
      confirmed: 'ad-status is-confirmed',
      shipped: 'ad-status is-shipped',
      delivered: 'ad-status is-delivered',
      cancelled: 'ad-status is-cancelled',
    };
    return map[status] ?? 'ad-status is-pending';
  }
  ...
  getPaymentStatusClasses(paymentStatus: string): string {
    const map: Record<string, string> = {
      pending: 'ad-status is-pending',
      paid: 'ad-status is-delivered',
      failed: 'ad-status is-cancelled',
      refunded: 'ad-status is-shipped',
    };
    return map[paymentStatus] ?? 'ad-status is-pending';
  }
}
