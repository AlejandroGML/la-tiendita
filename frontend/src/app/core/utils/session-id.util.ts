 ...
 */
/**
 * Generate a UUID v4 without relying on crypto.randomUUID, which is only
 * available in secure contexts (HTTPS / localhost). The demo runs over
 * plain HTTP, so fall back to a manual v4 construction.
 */
function generateUuid(): string {
  if (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function') {
    return crypto.randomUUID();
  }
  // Manual UUID v4 (RFC 4122) using crypto.getRandomValues when available.
